...
- Read first, in this order:
  ...
  4. `docs/LAUNCH_TASK_PROMPTS.md` — **the execution tracker for tasks T01–T22** (status table, pending founder decisions D1–D3, and a ready-made prompt per task). Backlog says WHAT is open; the tracker says HOW to execute it and what its current status is. When a backlog item has a T-number, work it via its prompt from this file.
...
2. Open `docs/LAUNCH_READINESS_BACKLOG.md` and work the highest-priority unchecked items in order — **announce which item you're starting, don't ask**, unless I named specific items in my message. **Stay within P0 by default**; move into P1/P2/P3 only when every actionable P0 item is complete or blocked on my input, or when I explicitly name a later item. When blocked on one item, note it and continue with the next workable item in scope, then present all blocked items together at the end. If I brought new content from the professional (adult questionnaire, videos/images), P2 jumps the queue after P0.1/P0.2.
   - **Tracker discipline (T-tasks):** if the item you pick has a T-number, open its prompt in `docs/LAUNCH_TASK_PROMPTS.md`, set its tracker status to 🔄, and follow the prompt exactly (role, scope, acceptance criteria, non-goals). Never mark ✅ without the prompt's verification steps observed. If founder decisions D1–D3 are still open, collect them and present the decision table once at the end of the session — do not stall workable tasks on them.
...
6. Before ending, update the **Next up** block at the top of the backlog to the 1–2 most important next actions, and update the status tracker in `docs/LAUNCH_TASK_PROMPTS.md` (✅ with evidence note / ⛔ with blocker) for every task you touched.
7. End of session: update `tasks/lessons.md` if I corrected you, tell me the single next most important item, and **print the full prompt of the next workable task verbatim** from `docs/LAUNCH_TASK_PROMPTS.md` (so I can paste it into a fresh session — or continue with it directly if this session has budget left). If the next step is a founder decision or founder-only action, print the decision table / click instructions instead.
