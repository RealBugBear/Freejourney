...
2. **D1–D3 ENTSCHIEDEN (Founder 2026-07-06):** D1=verstecken (Community/Feed → T04), D2=verstecken (Video → T06), D3=**Trainer-Discovery fertig bauen** (→ T13, jetzt vollwertiges Arbeitspaket). Founder-Auflage: Code+UI überall sauber nacharbeiten, **keine hässlichen Lücken**. Nächste Code-Tasks in Reihenfolge: **T04 → T06 → T07 → T13** (Prompts in `docs/LAUNCH_TASK_PROMPTS.md`). Konsequenz aus D3=B: Standort bleibt Launch-Datenkategorie → fließt in Nutrition Labels (T12), Consent (T05) und den Anwaltsauftrag (P0.6: Geolocation + OSM-Tileserver erwähnen).
...
- [ ] Umsetzung **T04**: Community-Tab + Experience-Feed verstecken, inkl. UI-Lücken-Pass (Founder-Auflage: keine leeren Sektionen/verwaisten CTAs — Mood-Check-in-Abschluss, „Begleitung“-Tab). Prompt: `docs/LAUNCH_TASK_PROMPTS.md` T04. Der Trainer-1:1-Chat bleibt; Kontaktweg wird in den Review-Notizen dokumentiert (T14).
...
- [ ] Umsetzung **T06**: Call-Buttons + IncomingCallListener hinter Flag, inkl. UI-Lücken-Pass (Chat-AppBar-Layout, Termin-Texte). Prompt: `docs/LAUNCH_TASK_PROMPTS.md` T06.
...
- [ ] **T13** Trainer-Discovery **fertig bauen** — Founder-Entscheidung D3 ✅ 2026-07-06: Feature bleibt im Launch („make it work“). Umfang laut Prompt (`docs/LAUNCH_TASK_PROMPTS.md` T13): nutzerinitiierte Standort-Abfrage mit allen Permission-Zweigen, ehrliche Empty-States (Launch-Realität: 0 freigeschaltete Trainer), OSM-Attribution auf Discovery- UND Location-Picker-Karte, Offline-/Fehlerzustände, Standort-Datenfluss-Doku als Input für Labels (T12), Consent (T05) und Anwaltsauftrag (P0.6).
...
- [ ] ASC: Privacy Nutrition Labels ausfüllen (E-Mail, Health & Fitness, User Content/Journal, Nutzungsdaten; kein Tracking; Sentry-Diagnostics ergänzen sobald eingebaut) — konsistent mit finaler Datenschutzerklärung (P0.6). **Update 2026-07-06 (T12, D-Entscheidungen eingearbeitet):** **Standort kommt fix dazu** (D3=B: Geolocator bei der Trainer-Suche + OSM-Tileserver; ob „erhoben“ oder transient, belegt der T13-Datenfluss-Abschnitt — T13 zuerst). Kamera/Mikrofon-DATEN kommen NICHT dazu (D2=A: Video versteckt; Permissions bleiben nur deklariert).
